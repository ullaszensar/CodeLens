...
class CodeAnalyzer:
    ...
    def __init__(self, repo_path: str, app_name: str):
        ...
        self.demographic_patterns = {
            'id':
            r'\b(customerId|cm_15)\b',
            'name':
            r'\b(first_name|last_name|full_name|name|amount)\b',
            'address':
            r'\b(address|street|city|state|zip|postal_code)\b',
            'contact':
            r'\b(phone|email|contact)\b',
            'identity':
            r'\b(ssn|social_security|tax_id|passport)\b',
            'demographics':
            r'\b(age|gender|dob|date_of_birth|nationality|ethnicity)\b'
        }

        self.integration_patterns = {
            'rest_api': {
                'http_methods':
                r'\b(get|post|put|delete|patch)\b.*\b(api|endpoint)\b',
                'url_patterns':
                r'https?://[^\s<>"]+|www\.[^\s<>"]+',
                'api_endpoints':
            ...
            'soap_services': {
                ...
                r'\b(soap|wsdl|xml)\b',
                'wsdl':
                r'wsdl|WSDL|\.wsdl|getWSDL|WebService[Client]?',
                'soap_operations':
                r'SOAPMessage|SOAPEnvelope|SOAPBody|SOAPHeader|SoapClient|SoapBinding',
                'xml_namespaces':
                r'xmlns[:=]|namespace|schemaLocation',
                'soap_annotations':
                r'@WebService|@WebMethod|@SOAPBinding|@WebResult|@WebParam',
                'soap_endpoints':
                r'endpoint[_\s]?url|service[_\s]?url|wsdl[_\s]?url'
            },
            'database': {
                'sql_operations':
                r'\b(select|insert|update|delete)\s+from|into\b',
                'db_connections':
                r'jdbc:|connection[_\s]?string|database[_\s]?url'
            },
            'messaging': {
            ...
            'file': {
                'file_operations':
                r'\b(csv|excel|xlsx|json|properties).*(read|write|load|save)\b'
            }
        }
    ...
    def analyze_file(self, file_path: Path) -> Dict:
        ...
        """
        results = {'demographic_data': {}, 'integration_patterns': []}
            ...
            for line_num, line in enumerate(content, 1):
                ...
                for data_type, pattern in self.demographic_patterns.items():
                    ...
                    for match in matches:
                        ...
                        if field_name not in results['demographic_data'][str(
                                file_path)]:
                            results['demographic_data'][str(
                                file_path)][field_name] = {
                                    'data_type': data_type,
                                    'occurrences': []
                                }
                        results['demographic_data'][str(
                            file_path)][field_name]['occurrences'].append({
                                'line_number':
                                line_num,
                                'code_snippet':
                                line.strip()
    ...
    def update_results(self, main_results: Dict, file_results: Dict,
                       file_path: Path):
        """  
        ...
        for file, fields in file_results['demographic_data'].items():
            if file not in main_results['demographic_data']:
                ...
                for field_name, data in fields.items():
                    if field_name not in main_results['demographic_data'][
                            file]:
                        main_results['demographic_data'][file][
                            field_name] = data
                    else:
                        main_results['demographic_data'][file][field_name][
                            'occurrences'].extend(data['occurrences'])
            demographic_fields_count += sum(
                len(data['occurrences']) for data in fields.values())
            main_results['summary']['unique_demographic_fields'].update(
                fields.keys())
        ...
        main_results['summary']['file_details'].append({
            ...
            demographic_fields_count,
            'integration_patterns_found':
            integration_patterns_count
        })
    ...
    def generate_report(self, results: Dict):
        ...
        results['summary']['unique_demographic_fields'] = list(
        ...
        self.logger.info(
            f"Analysis reports generated: {report_file} and {html_report}")
