...
# Page config
st.set_page_config(page_title="ZensarCA - Code Analysis",
                   page_icon="🔍",
                   layout="wide",
        ...
        try:
            with st.spinner("Analyzing code..."):
                ...
                with col1:
                    ...
                    stats_cols[0].metric("Files Analyzed",
                    ...
                    stats_cols[3].metric(
                        "Unique Fields",
                        len(results['summary']['unique_demographic_fields']))
                    ...
                    for file_path, fields in results['demographic_data'].items(
                        ...
                        with st.expander(f"📄 {os.path.basename(file_path)}"):
                            for field_name, data in fields.items():
                                st.write(
                                    f"**Field:** {field_name} ({data['data_type']})"
                                )
                    ...
                    for pattern in results['integration_patterns']:
                        with st.expander(
                                f"🔌 {pattern['pattern_type']} - {pattern['sub_type']}"
                        ):
                            st.write(
                                f"**File:** {os.path.basename(pattern['file_path'])}"
                            )
                ...
                with col2:
                    ...
                    report_files = [
                        ...
                        if f.endswith(('.html',
                                       '.json')) and 'code_analysis' in f
                    ]

                    for report_file in report_files:
                        st.markdown(get_file_download_link(
                            report_file, f"Download {report_file}"),
